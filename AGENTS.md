...
- Run `node build.mjs` after changes. It updates both `public/index.html` and the downloadable single-file demo.
...
- The published artifact is `public/` only. Do not expose project source files, local logs, or unrelated workspace files as static assets.
